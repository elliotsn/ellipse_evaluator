...
%
% Inputs:
%   shp     - cell array of polygon objects
...
%
% Outputs:
%   result  - a result object that contains result arrays, shp,
%             and other variables used for this evaluation.
%
%   Importantly:  
%
%     result.layerFrac - Array of size [nlayers numel(shp)], i.e. one slice
%                        for each layer. For placement of each polygon at
%                        ellLayerFrac(shp) is the fraction of the shape
%                        that is covered by the layer. This can be used as a
%                        metric of confidence, for example to show that even
%                        though 50% of the polygon is true for some layer,
%                        only 50% of the polygon is covered by the layer, so
%                        there is 50% area within the polygon with no data coverage, 
%                        where it is unknown if the layer meets the
%                        constraints or not.
%                 
%     result.layerTrueFrac  - Array of size [nlayers numel(shp)], i.e. one slice
%                             for each layer. It contains the fraction of
%                             each shape that meets the constraints
%                             for each layer.
%
%     result.trueFrac - Array the same size as numel(shp). For a polygon
%                       ellTrueFrac(shp) is the fraction of all pixels 
%                       in all layers that meet the defined numeric constraints 
%                       AND are within the polygon. This is a sum of
%                       layerTrueFrac down the layer dimension.
%
%  Copyright 2016  Elliot Sefton-Nash
%
function result = evaluatePolyCore(shp, layers, wb)
